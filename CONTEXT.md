...
| Name | Value | Role |
|------|-------|------|
| Periwinkle Beam | `linear-gradient(to right in oklab, rgb(44, 120, 252) 0%, rgb(145, 141, 246) 100%)` | Primary action background, active nav fills, brand icon strokes, decorative icon accents — the single chromatic action color across the system; Atmospheric section background transitioning from cobalt blue to periwinkle |
| Iris Dusk | `#9580ff` | Secondary violet variant for hover states and secondary action surfaces |
| Lavender Mist | `#dad9fc` | Soft tint for gentle background washes, subtle borders, and muted violet surfaces |
| Midnight Ink | `#181925` | Primary text, heading color, dark surface fill, high-contrast borders — the near-black that carries the entire information hierarchy |
| Graphite Veil | `#666666` | Hairline borders, icon strokes, muted dividers, secondary text — the dominant neutral border color |
| Ash | `#999999` | Tertiary text, inactive icon strokes, subtle UI lines |
| Fog | `#bbbcc3` | Light borders, inactive nav elements, low-emphasis dividers |
| Bone | `#e8e8e8` | Table dividers, table fills, very subtle separators |
| Paper | `#ffffff` | Page canvas, card surfaces, button text on dark fills, surface backgrounds |
| Mint Whisper | `#def6e4` | Soft section background, alternate surface, and quiet card fill |
| Signal Green | `#33c758` | Green outline accent for tags, dividers, and focused UI edges. Use as a supporting accent, not as a status color |
| Amber Spark | `#ffa600` | Yellow decorative accent for icons, marks, and small graphic details. Use as a supporting accent, not as a status color |
| Cobalt Pulse | `#2c78fc` | Violet accent for outlined action borders, linked labels, and lightweight interactive emphasis. Use as a supporting accent, not as a status color |
| Magenta Pop | `#d6409f` | Red decorative accent for icons, marks, and small graphic details. Use as a supporting accent, not as a status color |
| Ember | `#ff3e00` | Sparse accent for SVG illustrations and highlight decorations |
| Chart Blue | `linear-gradient(to right in oklab, rgb(190, 213, 254) 0%, rgb(235, 242, 255) 50%, rgb(145, 141, 246) 100%)` | Data sparkline gradient endpoint — light blue to cyan to periwinkle in dashboard previews |

## Typography

### OpenRunde — Single-family type system for all UI, body, headings, and navigation. Heavily tightened tracking at all sizes — -0.05em at 12px, -0.025em at 24px, -0.05em at 60px — gives the type a condensed, architectural density unusual for a product analytics brand.
- **Substitute:** Inter, Geist, or Satoshi
- **Weights:** 400, 500, 600, 700
- **Sizes:** 12px, 13px, 14px, 16px, 18px, 20px, 24px, 36px, 48px, 60px
- **Line height:** 1.00–1.56
...
| body | 14px | 1.5 | -0.38px |
| body-lg | 16px | 1.56 | -0.4px |
| subheading | 18px | 1.33 | -0.41px |
| heading-sm | 20px | 1.4 | -0.4px |
| heading | 24px | 1.4 | -0.43px |
| heading-lg | 36px | 1.17 | -0.58px |
| heading-xl | 48px | 1.13 | -0.62px |
| display | 60px | 1 | -0.55px |
...
- **Canvas** (`#ffffff`) — Page background, dominant surface
- **Card** (`#ffffff`) — Product preview, dashboard mockup, feature cards
- **Atmospheric Wash** (`#918df6`) — Blue-to-violet gradient bands behind product mockups
- **Highlight Tint** (`#def6e4`) — Soft green panels for status or highlight content
